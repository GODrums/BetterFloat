...
/**
 * Returns a pricempire link for a given item
 * Taken from https://github.com/gergelyszabo94/csgo-trader-extension/blob/b865e136473c941a20e0a982b4d41d8bdadc50d7/extension/src/utils/utilsModular.js#L969
 * @param itemType item type
 * @param itemName item name
 * @param dopplerType doppler type
 * @param condition condition
 * @returns pricempire link
 */
export function getPricempireLink(itemType: string, itemName: string, dopplerType: string, condition: string) {
		...
		switch (itemType) {
			case 'gloves':
				return `cs2-items/glove/${itemName
					...
					.toLowerCase()
					.replace(/\s+/g, '-')
					.replace(/[^\w-]+/g, '')}/${condition.replace(' ', '-').toLowerCase()}`;
			case 'sticker':
				...
				if (/\d{4}/.test(itemName)) {
					return `cs2-items/tournament-sticker/sticker-${itemName
						...
						.replace(/\s*\|\s*/g, '-')
						.toLowerCase()
						.replace(/\s+/g, '-')}/${(itemName.match(/\(\w+\)/) || [''])[0].replace(/[()]/g, '').toLowerCase()}`;
				}
			...
			case 'container':
				...
				if (/\d{4}/.test(itemName)) {
					...
					return (
						`cs2-items/tournament-team-sticker-capsule/${itemName
							.toLowerCase()
							.replace(/\s+/g, '-')
							.replace(/(\d{4})-/, '$1/')
							// no (holo/foil) in itemName still needs to be handled
							.replace(/\(([^/]+)\/([^/]+)\)/, '$1$2')}` + `-${type?.[1].toLowerCase()}`
					);
				}
				return `cs2-items/container/${itemName
					...
					.toLowerCase()}`;
			case 'custom_player':
				return `cs2-items/agent/${itemName
					.toLowerCase()
					.replace(/\s*\|\s*/g, '-')
					.replace(/\s+/g, '-')
					.replace(/[^\w-]+/g, '')}`;
			case 'music_kit':
				return `cs2-items/music-kit/${itemName
					...
					.replace(/\s+/g, '-')
					.toLowerCase()
					.replace(/[^\w-]+/g, '')}${itemName.includes('StatTrak') ? '/stattrak' : ''}`;
			case 'graffiti':
			...
			default:
				if (itemName.includes('★') && !itemName.includes('|')) {
					return `cs2-items/skin/${itemName.replace('StatTrak™ ', '').replace('★ ', '').replace(/\s+/g, '-').toLowerCase()}${itemName.includes('StatTrak') ? '/stattrak' : ''}`;
				}
				if (itemName.includes('Souvenir')) {
					return `cs2-items/skin/${itemName
						...
						.replace(/\s*\|\s*/g, '-')
						.toLowerCase()
						.replace(/\s+/g, '-')}/souvenir-${condition.replace(' ', '-').toLowerCase()}`;
				}
				return `cs2-items/skin/${itemName
					.replace('StatTrak™', '')
					.replace('★ ', '')
