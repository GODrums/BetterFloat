...
const ActivityBadge = ({ active }: { active: boolean }) => {
	return <div className={cn('px-2 pt-1 text-sm rounded font-medium', active ? 'bg-green-600 text-white' : 'bg-gray-600 text-gray-200')}>{active ? 'ON' : 'OFF'}</div>;
};

const SkbAutorefresh: React.FC = () => {
	const [open, setOpen] = useState(false);
	// auto-refresh
	const [isActive, setIsActive] = useState(false);
	const [rInterval, setRInterval] = useStorage('skb-autorefreshinterval');
	const [interval, setIntervalValue] = useState<NodeJS.Timeout | null>(null);

	const [user] = useStorage<SettingsUser>('user');

	const priceInput = document.querySelector<HTMLInputElement>('app-price-filter-new input[appnumbersonly]');
		...
		if (value) {
			const newInterval = setInterval(() => {
				...
				if (priceInput) {
					...
					const newValue = increase ? currentValue + 0.01 : currentValue - 0.01;
					priceInput.value = newValue.toFixed(2);

					// gain and lose focus to trigger update
					priceInput.focus();
	...
	useEffect(() => {
		const handleClickOutside = (event: MouseEvent) => {
			if ((event?.target as HTMLElement)?.tagName !== 'BETTERFLOAT-SKB-AUTOREFRESH') {
	...
	return (
		<div className="bg-transparent ml-6" style={{ fontFamily: 'Overpass, serif' }}>
			<Button
				className="h-10 px-4 flex items-center gap-2 bg-transparent border border-[#1d2530] hover:border-[#8293ac] rounded-lg text-white text-base font-medium transition-colors duration-200"
				onClick={toggleOpen}
			>
						...
						>
							...
							<div className="flex items-center justify-between">
								<div className="flex items-center gap-2">
									<MaterialSymbolsAvgTimeOutlineRounded className="h-6 w-6 text-gray-400" />
