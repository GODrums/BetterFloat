...
import { ICON_CSBLUEGEM, ICON_SKINSWAP_FULL } from '~lib/util/globals';
...
interface SkinswapSettingsProps {
	hasProPlan: boolean;
}

export const SkinswapSettings = ({ hasProPlan }: SkinswapSettingsProps) => {
	const [checked] = useStorage<boolean>('ss-enable');

	return (
		<TabTemplate value="ss" checked={checked}>
			{!hasProPlan && <WarningCallout text="Please upgrade to Pro to access Skinswap features" />}
			<MarketLogoFull icon={ICON_SKINSWAP_FULL} link="https://skinswap.com/r/betterfloat" />
			<div className="flex items-center justify-center gap-2">
				<Badge variant="outline">BETA</Badge>
			</div>
			<SettingsEnable id="ss-enable" isPremiumFeature hasProPlan={hasProPlan} />
			<div className="">
				...
				<div className="flex flex-col gap-1">
					...
					<SettingsCard>
						<SettingsCheckbox id="ss-stickerprices" text="Sticker Prices" icon={<PhSticker className="h-6 w-6" />} disabled />
					</SettingsCard>
				</div>
			</div>
			<div className="">
				...
				<div className="flex flex-col gap-1">
					<SettingsSource prefix="ss" />
					<SettingsCard>
						<SettingsCheckbox
							id="ss-buffdifference"
			...
			<div className="mb-2">
				...
				<div className="flex flex-col gap-1">
					<SettingsCard>
						<SettingsCheckbox id="ss-listingage" text="Show Listing Age" icon={<IcRoundAccessTime className="h-6 w-6" />} disabled />
					</SettingsCard>
				</div>
			</div>
		</TabTemplate>
