...
import { ICON_CSBLUEGEM, ICON_SKINFLOW_FULL } from '~lib/util/globals';
...
interface SkinflowSettingsProps {
	hasProPlan: boolean;
}

export const SkinflowSettings = ({ hasProPlan }: SkinflowSettingsProps) => {
	const [checked] = useStorage<boolean>('sf-enable');
	...
	return (
		<TabTemplate value="skinflow" checked={checked}>
			...
			<SettingsEnable id="sf-enable" isPremiumFeature hasProPlan={hasProPlan} />
			<div className="">
				...
				<div className="flex flex-col gap-1">
					...
					<SettingsCard>
						<SettingsCheckbox id="sf-stickerprices" text="Sticker Prices" icon={<PhSticker className="h-6 w-6" />} disabled />
					</SettingsCard>
				</div>
			</div>
			<div className="">
				...
				<div className="flex flex-col gap-1">
					<SettingsSource prefix="sf" />
					<SettingsCard>
						<SettingsCheckbox
							id="sf-buffdifference"
			...
			<div className="mb-2">
				...
				<div className="flex flex-col gap-1">
					<SettingsCard>
						<SettingsCheckbox id="sf-listingage" text="Show Listing Age" icon={<IcRoundAccessTime className="h-6 w-6" />} disabled />
					</SettingsCard>
				</div>
			</div>
		</TabTemplate>
