	...
	if (priceContainer && priceFromReference) {
		const newContainer = html`
			<div
				...
				style="background: linear-gradient(135deg, #0073d5, ${difference.isZero()
					? extensionSettings["sp-color-neutral"]
					: difference.isNeg()
					? extensionSettings["sp-color-profit"]
					: extensionSettings["sp-color-loss"]}); transform: skewX(-15deg); border-radius: 3px; padding-top: 2px;"
			>
	...
	if (item.price !== 0 && !isNaN(item.price) && saleTag && tooltipLink && !discountContainer.querySelector(".betterfloat-sale-tag") && (priceListing?.gt(0) || priceOrder?.gt(0))) {
		...
		const buffPriceHTML = html`
			<div style="${saleTagStyle}">
				${extensionSettings["sp-buffdifference"]
					? `<span>${difference.isPos() ? "+" : "-"}${item.currency}${difference.abs().toFixed(difference.gt(1000) ? 1 : 2)}</span>`
