...
async function init() {
	console.time('[BetterFloat] HaloSkins init timer');

	if (location.host !== 'haloskins.com') {
		return;
	}
	...
	if (!extensionSettings['hs-enable']) return;

	// check if user has the required plan
	if (!(await checkUserPlanPro(extensionSettings['user']))) {
		console.log('[BetterFloat] Pro plan required for HaloSkins features');
		return;
	}

	await initPriceMapping(extensionSettings, 'hs');

	console.timeEnd('[BetterFloat] HaloSkins init timer');
...
async function addBuffPrice(item: Skinout.Item | Skinout.InventoryItem, container: Element): Promise<PriceResult> {
	...
	if (
		...
		(extensionSettings['splace-buffdifference'] || extensionSettings['splace-buffdifferencepercent'])
	) {
...
async function getBuffItem(item: Skinout.Item | Skinout.InventoryItem) {
	...
	const referencePrice =
		Number(extensionSettings['splace-pricereference']) === 0 && ([MarketSource.Buff, MarketSource.Steam].includes(source) || (MarketSource.YouPin === source && isUserPro(extensionSettings['user'])))
			? priceOrder
			: priceListing;
